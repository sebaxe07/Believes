...
namespace BBUnity.Conditions {
    ...
    [Condition("Perception/SafetyIsTargetCloseAndInSight")]
    [Help("Checks whether a target is close and in sight depending on a given distance and an angle")]
    public class SafetyIsTargetCloseAndInSight : GOCondition {
		...
		public override bool Check() {
            ...
            //// Debug.DrawRay(gameObject.transform.position + new Vector3(0, 0.1f, 0), dir, Color.red, 1.0f);
            if (dir.sqrMagnitude > closeDistance * closeDistance)
                return false;
            //// Debug.DrawRay(gameObject.transform.position + new Vector3(0, 0.1f, 0), dir, Color.green, 1.0f);

            // Visualize the angle
            //Vector3 leftBoundary = Quaternion.Euler(0, -angle * 0.5f, 0) * gameObject.transform.forward * closeDistance;
            //Vector3 rightBoundary = Quaternion.Euler(0, angle * 0.5f, 0) * gameObject.transform.forward * closeDistance;
            //// Debug.DrawLine(gameObject.transform.position, gameObject.transform.position + leftBoundary, Color.blue, 1.0f);
            //// Debug.DrawLine(gameObject.transform.position, gameObject.transform.position + rightBoundary, Color.blue, 1.0f);
